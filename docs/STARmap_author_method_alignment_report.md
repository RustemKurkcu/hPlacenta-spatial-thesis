...
### Resume switch (implemented)

`cfg$resume$resume_from` supports:
- `week_qc` (default): run/refresh per-week loading+QC and continue all downstream stages.
- `merged`: start from `output/objects/01_merged_post_qc.rds`.
- `sct`: start from `output/objects/01_post_sct.rds`.
- `pca`: also starts from `output/objects/01_post_sct.rds`, then reruns PCA+Harmony+embeddings.
