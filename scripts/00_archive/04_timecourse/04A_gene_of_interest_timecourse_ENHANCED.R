...
# Use imputed for module scoring if available (better gene coverage)
if (has_imputed) {
  ...
  log_msg_enhanced("Using RNA_raw assay (imputed not available)", logfile, verbose = TRUE)
}

DefaultAssay(starmap) <- assay_starmap

# Ensure normalized
if (!("data" %in% Layers(starmap[[assay_starmap]]))) {
...
plot_gene_enhanced <- function(gene) {
  ...
  p <- summary_all %>%
    ...
    ggplot(aes(x = week, y = .data[[col]], color = celltype_use, group = celltype_use)) +
    ...
    labs(
      ...
      subtitle = "Mean expression per cell type and dataset",
      x = "Gestational Week", 
...
plot_score_enhanced <- function(score_name) {
  ...
  p <- summary_all %>%
    ...
    labs(
      ...
      subtitle = "Mean module score per cell type and dataset",
      x = "Gestational Week", 
