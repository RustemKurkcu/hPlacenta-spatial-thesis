...
resolve_features_with_aliases <- function(obj, genes, assay = NULL, gene_aliases = NULL,
  ...
  aliases <- gene_aliases %||% get0("GENE_ALIASES", ifnotfound = list(), inherits = TRUE)
  feature_pool <- if (isTRUE(case_insensitive)) toupper(feats) else feats
  
  matched <- character(0)
  map_rows <- list()
  
  for (g in genes) {
    candidates <- unique(c(g, unlist(aliases[[g]], use.names = FALSE)))
    candidates <- candidates[!is.na(candidates) & nzchar(candidates)]
    if (length(candidates) == 0) next
    
    candidate_pool <- if (isTRUE(case_insensitive)) toupper(candidates) else candidates
    hit_pos <- match(candidate_pool, feature_pool, nomatch = 0L)
    if (!any(hit_pos > 0L)) next
    
    first_hit <- which(hit_pos > 0L)[1]
    feat_hit <- feats[hit_pos[first_hit]]
    matched <- c(matched, feat_hit)
    map_rows[[length(map_rows) + 1L]] <- data.frame(
      requested_gene = g,
      matched_feature = feat_hit,
      matched_via = candidates[first_hit],
      stringsAsFactors = FALSE
    )
  }
  
  map_df <- if (length(map_rows) > 0) dplyr::bind_rows(map_rows) else {
    data.frame(requested_gene = character(0), matched_feature = character(0), matched_via = character(0), stringsAsFactors = FALSE)
  }
  
  found <- unique(matched)
  matched_requested <- if (nrow(map_df) > 0) unique(map_df$requested_gene) else character(0)
  missing <- setdiff(genes, matched_requested)
  
  list(found = found, missing = missing, map = map_df)
}
...
add_module_score_safe <- function(obj, genes, score_name,
  ...
  assay <- assay %||% Seurat::DefaultAssay(obj)
  feats <- rownames(obj[[assay]])
  resolved <- resolve_features_with_aliases(
    obj = obj,
    genes = genes,
    assay = assay,
  ...
  if (length(found) < min_genes) {
    ...
    attr(obj, "last_module_score_info") <- list(
      score_name = score_name,
      requested_n = length(unique(genes)),
      found_n = length(found),
      found_features = found,
      missing_genes = resolved$missing,
      matched_map = resolved$map,
      used_aliases = nrow(resolved$map) > 0 && any(resolved$map$requested_gene != resolved$map$matched_via)
    )
    return(obj)
  }
